...
class MovementComponent(Component):
        ...
        if self.behavior == 'fly' and self.player:
            ...
            if hasattr(self.node, 'input_vector'):
                ...
                if target_dir.length_squared() > 0:
                    ...
                    if self.velocity.length_squared() > 0 and target_dir.dot(self.velocity.normalize()) < -0.5:
                    ...
                    if self.velocity.length_squared() > 0:
                        ...
                        if self.velocity.length_squared() < friction_vec.length_squared():
    ...
    def fly_behavior(self, dt):
        ...
        if self.flee_timer > 0:
            ...
            if random.random() < 0.01:  # Occasionally change direction
                self.idle_direction = pygame.math.Vector2(random.choice([-1, 1]), random.choice([-1, 1]))
            dir_vector = self.idle_direction * self.max_speed * 0.2  # Slower idle movement
    ...
    def wander_chase_behavior(self, dt):
        if self.has_line_of_sight():  # Chase range
            ...
            if random.random() < 0.01:  # Occasionally change direction
                self.idle_direction = pygame.math.Vector2(random.choice([-1, 1]), random.choice([-1, 1]))
            dir_vector = self.idle_direction * self.max_speed * 0.5  # Slower
    ...
    def has_line_of_sight(self):
        ...
        search_rect = pygame.Rect(min(start.x, end.x), min(start.y, end.y), 
                                 abs(start.x - end.x), abs(start.y - end.y)).inflate(64, 64)
    ...
    def move(self, dir_vector, wall_tiles):
        if dir_vector.x == 0 and dir_vector.y == 0: return

        self.node.rect.x += dir_vector.x 
        if wall_tiles and not self.flying:
            for wall in wall_tiles:
                if abs(self.node.pos.x - wall.rect.centerx) > 64: continue
        ...
        if wall_tiles and not self.flying:
            for wall in wall_tiles:
                if abs(self.node.pos.y - wall.rect.centery) > 64: continue
                if self.node.rect.colliderect(wall.rect):
